...
变速箱类型检测增加"档位是否动过"判断，防止中文输入法拦截换挡键导致误判。

- **问题**：中文输入法下换挡键被 IME 吞掉，gear 纹丝不动 → 2 秒超时 → 误判为"离合箱"
- **修复**：超时时先检查 gear 是否离开过起始档位（包括弹到空档 11）
  - gear 动过 → 游戏收到了换挡 → 按正常逻辑判 clutch
  - gear 没动过 → 换挡键没到游戏（IME/失焦）→ 不分类，下次升档重试

### 降档补油日志

离合辅助下降档补油操作现在输出日志 `[RevBlip]`，方便观察补油的行为。

| 文件 | 改动 |
|------|------|
| `virtual_tcu/logic/tcu.py` | 新增 `_detect_gear_moved` 标志，超时分支分两种路径 |
| `virtual_tcu/input/keyboard_output.py` | 新增 `[RevBlip]` 日志输出 |
