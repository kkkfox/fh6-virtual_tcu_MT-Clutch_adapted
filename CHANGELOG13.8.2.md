...
| `virtual_tcu/logic/tcu.py` | 换挡方式检测（单阈值 `≤5`）, `_shift_to()` 分流, profile 持久化, `_split_tune_profile` 继承检测结果, snapshot 推送 `clutch_assist_enabled` / `transmission_type`, `_resolve_pending_upshift` IME 干扰修复 |
