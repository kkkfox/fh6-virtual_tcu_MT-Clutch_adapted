<script setup lang="ts">
  ...
  const transBadge = () => {
    const tt = telemetry.value?.transmission_type
    if (tt === 'sequential') return 'text-success bg-success/10 border-success/30'
    if (tt === 'clutch') return 'text-warn bg-warn/10 border-warn/30'
    return 'text-tcu-txt-dim bg-tcu-bg-3 border-tcu-border'
  }
  const transText = () => {
    const tt = telemetry.value?.transmission_type
    if (tt === 'sequential') return t('calibration.transTypeSequential')
    if (tt === 'clutch') return t('calibration.transTypeClutch')
...
<template>
  <div :class="col">
    ...
    <div :class="cardSm">
      ...
      <div v-if="telemetry?.clutch_assist_enabled" class="mt-2 flex items-center justify-between gap-2">
