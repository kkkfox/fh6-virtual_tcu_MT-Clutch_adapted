...
| | |
| :-- | :-- |
| 🚗 **四种驾驶模式 + 手动** | 舒适 · 竞技 · 漂移 · 越野（舒适/竞技随驾驶风格自适应） |
...
| 模式 | 说明 |
|------|------|
| **COMFORT（舒适）** | 省油取向；开启**驾驶风格**后，在巡航省油、自适应保持与更运动响应间切换（取代原独立 DYNAMIC 模式） |
| **RACE（竞技）** | 接近红线升档、积极降挡与发动机制动；激烈驾驶时驾驶风格可进一步推高运动性 |
| **DRIFT（漂移）** | 维持动力区间转速，减少漂移中意外降档 |
| **OFFROAD（越野）** | 适应低抓地与起伏路况 |
| **MANUAL（手动）** | 关闭 TCU 自动换挡 |

**F9** 循环：**舒适 → 竞技 → 漂移 → 越野 → 手动**。**F8** 切换日志录制。可在设置窗口 / Web UI 中修改。
...
**设置 → 难度 → 变速箱 → 手动（无离合器）** — 若希望 TCU 在换挡时自动按离合，请在 Virtual TCU 中开启**离合辅助**（键盘或 vJoy）。
...
| **HUD 悬浮窗** | 置顶 overlay，**经典 / 竞技 / 极简** 三模板 — 弧形或分段转速表、换档箭头、踏板条；支持点击穿透与固定 |
